function GetParameterValues(viewer) {
    var parameters = viewer.GetParameters().GetParameterList();
    var data = [];
    $.each(parameters, function (_, p) {
        data.push({ Name: p.GetName(), Value: p.GetValue() });
    });
    return data
}
